...
struct Path
{
	...
	Path<_Ty>(const _Ty&to,int len)noexcept
	{
		this->to=to;
		this->len=len;
	}
	Path<_Ty>(const _Ty&to)noexcept
	{
		this->to=to;
	}
	Path<_Ty>()noexcept
	...
	bool operator<(const Path<_Ty>&other)const noexcept
	{
		return to<other.to;
	}
	bool operator>(const Path<_Ty>&other)const noexcept
...
public:
	...
	void connect(const _Ty&from,const _Ty&to,int len,bool isOneWay=true)noexcept
	{
		insert(from);
		insert(to);
	...
	void connect(const _Ty&from,const Path<_Ty>&path)noexcept
	{
	...
	std::vector<Path<_Ty>>&find(const _Ty&id)noexcept
	{
		return vertecies[id];
	}
 
 
	std::vector<_Ty>listVertecies()noexcept
	{
	...
	std::vector<std::pair<std::pair<_Ty,_Ty>,int>>listPaths()noexcept
	{
	...
	std::map<_Ty,int>dijkstra(const _Ty&from)noexcept
	{
	...
	std::map<_Ty,int>dijkstra(const _Ty&from,const _Comp&greaterThanOrEqualTo)noexcept
	{
	...
	std::map<_Ty,int>dijkstra(const _Ty&from,
...
private:
	...
	void insert(const _Ty&id)noexcept
	{
...
struct Tree
{
	Tree(const _Ty&node,const std::vector<Tree<_Ty>>&subnodes)noexcept
	{
		this->subnodes.assign(subnodes.begin(),subnodes.end());
		this->node=node;
	}
	Tree(const _Ty&node)noexcept
	{
		this->node=node;
	}
	Tree()noexcept
...
public:
	...
	SegTree(const _Func&func)noexcept
	{
		initialize(func);
	}
	template<typename _Func>
	SegTree(const int&size,const _Func&func)noexcept
	{
		initialize(size,func);
	}
	template<typename _Iterator,typename _Func>
	SegTree(_Iterator begin,_Iterator end,const _Func&func)
	{
		if(begin>end)
			throw 1;
	...
	void update(int idx,const _Ty&val)noexcept
	{
	...
	template<typename _Func>
	void initialize(const _Func&func)noexcept
	{
		this->func=func;
		data.clear();
		return;
	...
	template<typename _Func>
	void initialize(const int&size,const _Func&func)noexcept
	{
		_Ty allocator[size];
	...
	void initialize(_Iterator begin,_Iterator end,const _Func&func)
	{
		if(begin>end)
			throw 1;
		this->func=func;
...
private:
	...
	void updateFwd(const int&idx,int first,int last,int pos,const _Ty&val)noexcept
	{
		if(first==last)
		{
			data[pos]=val;
	...
	void initFwd(_Iterator begin,int first,int last,int pos)noexcept
	{
		if(first==last)
		{
	...
	_Ty queryFwd(const int&first,const int&last,int ptrFirst,int ptrLast,int pos)noexcept
	{
		if(ptrFirst==ptrLast)
			return data[pos];
...
void myCodeHere()
{
	...
	rep(q)
	{
		int y1=getInt()-2,x1=getInt()-2,y2=getInt()-1,x2=getInt()-1;
 
		int topLeft=y1<0?0:x1<0?0:arr[y1][x1];
		int bottomRight=arr[y2][x2];
		int top=y1<0?0:arr[y1][x2];
		int left=x1<0?0:arr[y2][x1];
		println(bottomRight-top-left+topLeft);
	}
}
