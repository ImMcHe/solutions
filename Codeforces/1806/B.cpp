...
void printFwd(const bool&isOut,const bool&val)
{
	if(isOut)
    {
        if(isEnd_printFwd)
...
{
	if(isFirstGet_getFwd)
	{
		cin.ignore();
		cin.clear();
	}
	isFirstGet_getFwd=false;
...
int n,dp[MAX_VAL],numOfGreaterThan0;
...
void myCodeHere()
{
	...
	if(numOfGreaterThan0==0)
		put(1);
	elif(numOfGreaterThan0+1<dp[0])
	{
		if(numOfGreaterThan0==dp[1])
			put(2);
		else
			put(1);
	}
